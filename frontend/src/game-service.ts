...
export function initiateNewChessGame(store: Store<any>, pgn?: string) {
	let chess = new Chess()
	let newPgn: string

	if (pgn) {
		const cleanedPgn = pgn.replace(/\{[^}]+\}/g, '').replace(/\[.*?\]/g, '')
		let pgnWithoutMetadata = cleanedPgn.trim()
		newPgn = pgnWithoutMetadata
	} else {
		const defaultPgn =
	...
	store.commit('setTargetPgn', newPgn)
	store.commit('resetFen')
	store.commit('newChessGame', chess)
}
